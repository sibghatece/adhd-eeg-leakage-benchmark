%% stage3b_evaluate_multichannel.m
%  STAGE 3b - Leakage-free evaluation on Stage 2b multichannel feature stores.
%
...
IN = fullfile(ROOT, 'Stage2b_Features'); OUT = fullfile(ROOT, 'Stage3b_Results');
...
L('STAGE 3b - MULTICHANNEL EVALUATION   %s', datestr(now, 'yyyy-mm-dd HH:MM:SS'));
...
cohorts(end+1) = struct('name', 'D1_IEEE_CzF4',      'F', F1,  'chan', {{'Cz','F4'}});
cohorts(end+1) = struct('name', 'D2m_TDBRAIN_26ch',  'F', F2m, 'chan', {F2m.channels});
cohorts(end+1) = struct('name', 'D2m_TDBRAIN_CzF4',  'F', F2m, 'chan', {{'Cz','F4'}});
