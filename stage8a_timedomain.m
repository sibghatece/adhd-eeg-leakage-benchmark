%% stage8a_timedomain.m
%  STAGE 8a - Time-domain / Hjorth family (7 features per channel):
%     logvar, Hjorth mobility, Hjorth complexity, skewness, kurtosis, zero-crossing rate, line length
%  Reads every D*_epochs.mat in Stage1b_Epochs; writes Stage8_Features\timedomain\D*_timedomain.mat
%  Feature store F: feat [nEp x nFeat], featNames, channels, subject, subjectID, label, age, sex, dataset, family

clear; clc;
ROOT = 'C:\Users\Admin\Documents\WROK_WORK_WORK\Next_phase_Mental_Disorders\3_Problem_3_ADHD_ThreeDatasets+Paper';
FAMILY = 'timedomain'; PER_CH_NAMES = {'logvar', 'mobility', 'complexity', 'skewness', 'kurtosis', 'zcr', 'linelength'};
s8_run(ROOT, FAMILY, PER_CH_NAMES, @featTD);

function f = featTD(x, ~)
    % x: column vector (one epoch, one channel, physical units)
    dx = diff(x); ddx = diff(dx);
    v0 = var(x); v1 = var(dx); v2 = var(ddx);
    mob = sqrt(v1 / max(v0, eps)); cplx = sqrt(v2 / max(v1, eps)) / max(mob, eps);
    f = [log(v0 + eps), mob, cplx, skewness(x), kurtosis(x), mean(diff(sign(x)) ~= 0), mean(abs(dx))];
end
